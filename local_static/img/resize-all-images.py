...
#   R                 - right pointer
#
def resize_img(file, name, dimension, dimension_factor, rotation, file_ext, file_ext_alias, i, quality, L, R):
  ...
  if quality <= 95:
    ...
    if os.path.getsize(file_output_path + new_name) < size_target:
      ...
      L = quality
      quality = int((R + L) / 2)
      resize_img(file, name, dimension, dimension_factor, rotation, file_ext, file_ext_alias, i + 1, quality, L, R)
...
def create_resized_img(file, name, dimension, dimension_factor, rotation, file_ext, file_ext_alias, i, quality, L, R):
  try: 
    print('=== Resizing: {}, {}, {}, {}x'.format(file, dimension, file_ext, dimension_factor))
    resize_img(file, name, dimension, dimension_factor, rotation, file_ext, file_ext_alias, i, quality, L, R)
  except IOError as e:
...
for root, dirs, files in os.walk(file_input_path, topdown=False):
  for file in files:
    ...
    if os.path.splitext(file)[1] != '.py':
      ...
      #
      # Syntax:
      # resize_img(file, name, dimension, dimension_factor, rotation, ext, i, quality, L, R)

      # Call the create_resized_img() function, specifying arguments to pass to
      # the dimension_factor, rotation, file_ext, file_ext_alias parameters.
      #
      # Syntax:
      # create_resized_img(file, name, dimension, dimension_factor, rotation, file_ext, file_ext_alias, i, quality, L, R)
